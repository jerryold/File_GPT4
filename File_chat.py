...
#     llm = ChatOpenAI(model='gpt-4', temperature=1,max_tokens=4096,frequency_penalty=0)
...
def ask_and_get_answer_query(vector_store, q, k=3):
    from langchain.chains import RetrievalQA
    from langchain.chat_models import ChatOpenAI
...
if __name__ == "__main__":
    ...
    with open('./cookies_snapshot/api.json') as f:
        data = json.load(f)
    ...
    col1.image('images/ssstc.png')
    col2.image('images/langchain-chroma-light.png')


    st.header('SSSTC File Question-Answering Application')
    with st.sidebar:
    ...
    if uploaded_files and 'vs' in st.session_state: #and is_api_key_valid(api_key):
        ...
        if q: # if the user entered a question and hit enter
            if 'vs' in st.session_state: # if vector store exists in the session state
                ...
                answer,source= ask_and_get_answer_query(vector_store, q, k)
                ...
                for i in range(len(source)):
                    with st.expander(f'Source {i+1}'):
                        st.text_area('', value=source[i], height=20)
        ...
        # # text area widget for the source of the question
    ...
    # Wrap the annotated text in a div with the center class
   

    annotated_text(("Developed by","SDT"))

    
    # insert subtitle
    # st.subheader('Develop by SDT')
    # st.markdown('''
